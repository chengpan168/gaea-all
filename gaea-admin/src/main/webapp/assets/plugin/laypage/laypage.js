...
//分页视图
    Page.prototype.view = function(){
        ...
        if(conf.curr > 1 && conf.prev){
            view.push('<a href="javascript:;" class="ui basic icon button tiny" data-page="'+ (conf.curr - 1) +'"><i class="left chevron icon"></i></a>');
            //view.push('<a href="javascript:;" class="laypage_prev" data-page="'+ (conf.curr - 1) +'">'+ conf.prev +'</a>');
        }

        //当前组非首组，则输出首页
        if(dict.index > 1 && conf.first && conf.groups !== 0){
            view.push('<a href="javascript:;" class="ui basic icon button tiny" data-page="1"> 1 </a><span>&#x2026;</span>');
        ...
        for(; dict.start <= dict.end; dict.start++){
            if(dict.start === conf.curr){
                view.push('<span  class="ui basic icon button tiny active" >'+ dict.start +'</span>');
                //view.push('<span class="laypage_curr" '+ (/^#/.test(conf.skin) ? 'style="background-color:'+ conf.skin +'"' : '') +'>'+ dict.start +'</span>');
            } else {
                view.push('<a href="javascript:;" class="ui basic icon button tiny" data-page="'+ dict.start +'">'+ dict.start +'</a>');
        ...
        if(conf.pages > conf.groups && dict.end < conf.pages && conf.last && conf.groups !== 0){
            view.push('<span>&#x2026;</span><a href="javascript:;" class="ui basic icon button tiny" data-page="'+ conf.pages +'">' + conf.pages + '</a>');
        ...
        if(conf.curr !== conf.pages && conf.next || dict.flow){
            view.push((function(){
                return '<a href="javascript:;" class="ui basic icon button tiny" data-page="'+ (conf.curr + 1) +'"><i class="right chevron icon"></i></a>';
        ...
        var pageDesc = '<div class="eight wide column page-info"><label>显示第 {0} 到第 {1} 条记录，总共 {2} 条记录</label></div>'
            .format((conf.curr - 1) * conf.pageSize + 1, conf.curr * conf.pageSize, conf.totalCount);
        return pageDesc + '<div class="eight wide column page"><div name="laypage'+ laypage.v +'" class="laypage" id="laypage_'+ that.config.item +'">'+ view.join('') + function(){
                return conf.skip
